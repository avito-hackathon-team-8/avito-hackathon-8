...
func getConfig(t *testing.T) testConfig {
	...
	if cfgErr != nil {
		t.Skipf("shop API tests need running backend and compose postgres: %v", cfgErr)
	}
...
func prepareConfig() (testConfig, error) {
	...
	if probe.status != http.StatusUnauthorized {
		return testConfig{}, fmt.Errorf("running backend does not expose current shop API: GET /api/v1/shop returned %d", probe.status)
	}
