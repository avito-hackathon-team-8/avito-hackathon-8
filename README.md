...
Уровень определяется сервером по состоянию питомца. При успешном `claim`
награда в листьях и статус задания записываются в одной транзакции.
...
WebSocket отправляет начальное состояние после подключения и событие
`PET_PROGRESS_UPDATED` после начисления листьев. Имя питомца должно содержать
от 1 до 35 символов после удаления пробелов по краям.

## API и ограничения MVP

Полный контракт запросов, ответов и ошибок приведён в
[docs/openapi.yaml](docs/openapi.yaml). Бизнес-правила задач и питомца описаны
в [docs/business_logic](docs/business_logic).

`POST /api/v1/tasks/record` принимает уже подтверждённые события. В MVP
backend не получает эти события напрямую из внешних сервисов Авито: их должен
передавать клиент или интеграционный слой.
