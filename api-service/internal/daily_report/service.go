...
func leafActivityForDay(tx *gorm.DB, userID uuid.UUID, dayStart, dayEnd time.Time) (int, *LevelUp, time.Time, error) {
	...
	err := tx.Model(&models.LeafTransaction{}).Select(`
		COALESCE(SUM(CASE WHEN amount > 0 AND reason IN (?, ?, ?) THEN amount ELSE 0 END), 0) AS earned_leaves,
		COUNT(*) FILTER (WHERE reason = ?) AS level_ups`, models.LeafReasonTaskReward, models.LeafReasonWeeklyLogin, models.LeafReasonMVP, models.LeafReasonLevelUp).
		Where("user_id = ? AND occurred_at >= ? AND occurred_at < ? AND reason IN ?", userID, dayStart, dayEnd, []models.LeafTransactionReason{models.LeafReasonTaskReward, models.LeafReasonWeeklyLogin, models.LeafReasonMVP, models.LeafReasonLevelUp, models.LeafReasonChestPurchase}).Scan(&aggregate).Error

	if err != nil {
		return 0, nil, time.Time{}, fmt.Errorf("aggregate leaf activity: %w", err)
	}

	earnedLeaves := aggregate.EarnedLeaves
	updatedAt := time.Time{}

	var latest models.LeafTransaction

	baseQuery := tx.Select("occurred_at").Where("user_id = ? AND occurred_at >= ? AND occurred_at < ? AND reason IN ?", userID, dayStart, dayEnd, []models.LeafTransactionReason{models.LeafReasonTaskReward, models.LeafReasonWeeklyLogin, models.LeafReasonMVP, models.LeafReasonLevelUp, models.LeafReasonChestPurchase})

	if err := baseQuery.Order("occurred_at DESC").First(&latest).Error; err == nil {
		updatedAt = latest.OccurredAt.UTC()
	} else if !errors.Is(err, gorm.ErrRecordNotFound) {
		return 0, nil, time.Time{}, fmt.Errorf("get latest leaf activity: %w", err)
	}
	...
	if aggregate.LevelUps == 0 {
		return int(earnedLeaves), nil, updatedAt, nil
	}

	var latestLevelUp models.LeafTransaction

	if err := tx.Select("occurred_at").Where("user_id = ? AND occurred_at >= ? AND occurred_at < ? AND reason = ?", userID, dayStart, dayEnd, models.LeafReasonLevelUp).Order("occurred_at DESC").First(&latestLevelUp).Error; err != nil {
		return 0, nil, time.Time{}, fmt.Errorf("get latest level up: %w", err)
	}
	...
	fromLevel := pet.Level - aggregate.LevelUps

	if fromLevel < 1 {
		return 0, nil, time.Time{}, fmt.Errorf("calculate level up: current level %d is inconsistent with %d level-up transactions", pet.Level, aggregate.LevelUps)
	}

	return int(earnedLeaves), &LevelUp{
		FromLevel:  fromLevel,
		ToLevel:    pet.Level,
		OccurredAt: latestLevelUp.OccurredAt.UTC(),
	}, updatedAt, nil
}
