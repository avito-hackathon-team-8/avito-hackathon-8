...
func (service *Service) CreditTx(tx *gorm.DB, credit Credit) (Progress, error) {
	...
	periodStart := time.Date(credit.OccurredAt.Year(), credit.OccurredAt.Month(), 1, 0, 0, 0, 0, time.UTC)
	total := models.LeaderboardTotal{PeriodStart: periodStart, UserID: credit.UserID, Leaves: credit.Amount, UpdatedAt: credit.OccurredAt}
	if err := tx.Clauses(clause.OnConflict{Columns: []clause.Column{{Name: "period_start"}, {Name: "user_id"}}, DoUpdates: clause.Assignments(map[string]any{"leaves": gorm.Expr("leaderboard_totals.leaves + EXCLUDED.leaves"), "updated_at": credit.OccurredAt})}).Create(&total).Error; err != nil {
		return Progress{}, fmt.Errorf("update leaderboard total: %w", err)
	}
