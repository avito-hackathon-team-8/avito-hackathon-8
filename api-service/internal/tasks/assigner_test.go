...
func TestDailyTasksAssignerRetriesOneServerFailure(t *testing.T) {
	var calls atomic.Int32
	token := strings.Repeat("t", 32)
	assigner := NewDailyTasksAssigner("http://daily-tasks-service.test", token)
...
func TestDailyTasksAssignerDoesNotRetryClientFailure(t *testing.T) {
	var calls atomic.Int32
	assigner := NewDailyTasksAssigner("http://daily-tasks-service.test", strings.Repeat("t", 32))
